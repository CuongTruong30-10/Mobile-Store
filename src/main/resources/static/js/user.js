async function login() {
    ...
    var user = { // Tạo ra user đăng nhập
        "username": username,
        "password": password,
    }
    //Gọi Api và truyền vào user
    ...
    //Trả về kết quả là 1 Json
    var result = await response.json();
    if (response.status < 300) {
    //Nếu thành công lưu tt user và token vào LocalStorage
        localStorage.setItem("user", JSON.stringify(result.user));
        localStorage.setItem("token", result.token);
        // Check các quyền để load vào trang
